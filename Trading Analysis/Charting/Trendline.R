...
ticksize = "Weekly"
weekly.result = getSRLevels(ticksize, start.date = start.weekly, 
                           days = 10, duration = 10, tickers= tickers.alert)
weekly.output = weekly.result[,c(5,3,6,2,7,1)]
colnames(weekly.output) = c("sup1","res1","sup2","res2","sup3","res3")
weekly.output[is.na(weekly.output)] <- 0

write.csv(weekly.output,"weekly_levels.csv")
